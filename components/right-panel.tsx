...
// Simple 3D cube that rotates based on gyro values with smooth interpolation
function GyroCube({ gyro }: { gyro: GyroVec }) {
	const meshRef = useRef<THREE.Mesh>(null)
	const targetQuatRef = useRef(new THREE.Quaternion())
	const currentQuatRef = useRef(new THREE.Quaternion())
	const prevGyroRef = useRef<GyroVec>({ x: 0, y: 0, z: 0 })

	// Update target rotation ONLY when gyro values actually change
	useEffect(() => {
		// Check if gyro values have changed
		if (
			prevGyroRef.current.x === gyro.x &&
			prevGyroRef.current.y === gyro.y &&
			prevGyroRef.current.z === gyro.z
		) {
			return // No change, don't update
		}

		// Store new gyro values
		prevGyroRef.current = { x: gyro.x, y: gyro.y, z: gyro.z }

		// Only update target when data changes
		const SCALE = 0.015
		const euler = new THREE.Euler(gyro.x * SCALE, gyro.y * SCALE, gyro.z * SCALE, "XYZ")
		const quat = new THREE.Quaternion()
		quat.setFromEuler(euler)
		targetQuatRef.current = quat
	}, [gyro.x, gyro.y, gyro.z])

	// Smoothly interpolate rotation each frame
	useFrame(() => {
		if (!meshRef.current) return

		// Slerp (spherical linear interpolation) for smooth quaternion rotation
		currentQuatRef.current.slerp(targetQuatRef.current, 0.1) // 0.1 = 10% smoothing factor
					...
					{alerts.length === 0 ? (
						...
						alerts.slice(0, 5).map((alert, i) => (
							<div key={i} className="text-xs flex justify-between items-start pb-2 border-b border-border">
								...
								<span
									className={`px-2 py-1 rounded text-xs font-semibold whitespace-nowrap ml-2 ${alert.severity === "critical"
